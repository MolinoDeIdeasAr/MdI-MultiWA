...
async function ejecutar(instanceId) {
    ...
    //----------------------------------------------------------
    // OK — usar resultado.pausa de antiBan.getPausaAleatoria()
    //----------------------------------------------------------
    if (resultado.tipo === runner.RESULTADO.OK) {
        guardarEstadoSeguro(instanceId);

        // FIX: usar resultado.pausa que viene de la config antiban
        // en lugar de generarDelayHumano que ignora la configuración
        const pausa = resultado.pausa || 120000;
...
function pausar(instanceId) {
    detener(instanceId);
    console.log(`⏸ Scheduler pausado (${instanceId})`);
}
